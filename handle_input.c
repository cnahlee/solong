...
void	ft_player_move(t_game *game, int new_y, int new_x, int player_sprite)
{
	...
	if (game->map.map[new_y][new_x] == MAP_EXIT && game->map.coins == 0)
		ft_victory(game);
	else if ((game->map.map[new_y][new_x] == FLOOR)
	|| (game->map.map[new_y][new_x] == COINS))
	{
		game->map.map[last_y][last_x] = FLOOR;
		if (game->map.map[new_y][new_x] == COINS)
			game->map.coins--;
